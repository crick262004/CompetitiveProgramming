...
struct FenwickTree {
    ...
    FenwickTree(const vector<int> &a) : FenwickTree((int)a.size()) {
        for (size_t i = 0; i < a.size(); i++) add((int)i, a[i]);
    }
...
struct FenwickTreeMin {
    ...
    FenwickTreeMin(vector<int> a) : FenwickTreeMin((int)a.size()) {
        for (size_t i = 0; i < a.size(); i++) update((int)i, a[i]);
    }
...
struct FenwickTreeOneBasedIndexing {
    ...
    FenwickTreeOneBasedIndexing(vector<int> a)
        : FenwickTreeOneBasedIndexing((int)a.size()) {
        for (size_t i = 0; i < a.size(); i++) add((int)i, a[i]);
    }
