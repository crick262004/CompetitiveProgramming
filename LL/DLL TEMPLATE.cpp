...
Node* insertAtTail(Node* head, int k) {
    ...
    Node* newNode = new Node(k);  // If the doubly linked list is empty, set 'head' to the new node
    if (head == nullptr) {
        return newNode;
    }
...
Node* deleteHead(Node* head) {
   ...
   // Store the current head as 'prev'
    ...
    head = head->next;      // Set 'back' pointer of the new head to nullptr
    head->back = nullptr;     // Set 'next' pointer of 'prev' to nullptr
    prev->next = nullptr;    // Return the new head
    return head;          
}
...
Node* deleteTail(Node* head) {
    ...
    tail->back = nullptr;  // Free memory of the deleted node
    delete tail;  
...
Node* reverseDLL(Node* head) {  // Check if the list is empty
    ...
    if (head == NULL || head->next == NULL) {
     ...
     // the previous node
    Node* prev = NULL;    // Initialize a pointer to
    // the current node
    Node* current = head;     // Traverse the linked list
    while (current != NULL) {
        // Store a reference to
        // the previous node
        prev = current->back;   // Swap the previous and
        // next pointers
        current->back = current->next;   // This step reverses the links
