...
This note stores a compressed subarray at each node. It is useful when there is no update operation and the query can be answered by combining only the $O(\log N)$ covered nodes.
