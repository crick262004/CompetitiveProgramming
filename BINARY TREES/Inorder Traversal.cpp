...
public:
    ...
    // { //     if(root == NULL)
    //         return; //     util(root->left, ans);
    //     ans.push_back(root->val); //     util(root->right, ans);
    //     return; // }
    vector<int> inorderTraversal(TreeNode* root) { // vector<int> ans;
        // util(root, ans); // return ans;
        // iterative solution:
        // vector<int> ans; // stack<TreeNode*> st;
        // TreeNode* temp = root; // while(temp)
        // { //     st.push(temp);
        //     temp = temp->left; // }
        // while(!st.empty())
        // { //     TreeNode* cur = st.top();
        //     st.pop(); //     ans.push_back(cur->val);
        //     temp = cur->right; //     while(temp)
        //     { //         st.push(temp);
        //         temp = temp->left; //     }
        // }
        // return ans; // ITERATIVE STANDARD SOLUTION
        ...
        // vector<int> ans; // TreeNode* temp = root;
        // stack<TreeNode*> st; // while(true)
        // { //     if(temp)
        //     { //         st.push(temp);
        //         temp = temp->left; //     }
        //     else
        //     { //         if(st.empty())
        //             break; //         TreeNode* node = st.top();
        //         st.pop(); //         ans.push_back(node->val);
        //         temp = node->right; //     }
        // }
        // return ans; // the morris traversal.
        // O(1) space 
